...
//Code Here
let totalPopulation = populations.reduce((total, current) => total += current, 0);
...
let ordersTotal = orders.map(ObjElement => (ObjElement.price * ObjElement.tax) + ObjElement.price)
...
let bobsTotal = purchases.filter(x => x.owner === 'Bob').reduce((total, current) => total += current.price, 0)
